...
const placeorder = async (req, res) => {
    try {
        ...
        const orderedData = new Date();
        const expectedArrivalDate = new Date(orderedData);
        expectedArrivalDate.setDate(orderedData.getDate() + shippingTime);
        ...
        if (selectedPaymentMethod === 'Cash on delivery') {
            const order = new orderModel({
                ...
                deliveredAt : formattedArrivalDate
            });
            ...
            try {
                ...
                // Verify the wallet save
                const updatedWallet = await Wallet.findOne({ wallet_user: userId });
                ...
                const walletOrder = new orderModel({
                    ...
                    totalAmount: totalCart,
                    address: address,
                    deliveredDate: formattedArrivalDate,
                    OrderedDate : formattedDate,
                    deliveredAt : formattedArrivalDate
                });
            ...
            const orderOnline = new orderModel({
                ...
                totalAmount: totalCart,
                address: address,
                deliverdDate: formattedArrivalDate,
            ...
            orderOnline.paymentStatus = "success"
            ...
            try {
                ...
                return res.status(500).json({ message: 'Error processing Razorpay order', error });
            }
        } else {
...
const verifyRazorpay = async (req, res) => {
    ...
    try {
        ...
        const foundOrder = await orderModel.findOne({ userId: userId, orderId: razorpayOrderId });
...
const checkoutPage = async (req, res) => {
    try {
        ...
        if (usercart) {
            usercart.products = usercart.products.filter(product => product.isSelected);

            if (usercart.products.length === 0) {
                return res.render('user/checkout', { message: 'No products selected for checkout', userLogged });
            }
            ...
            if (cartTotal >= 4000 && cartTotal <= 6000) {
                console.log('>=4000')
                availableCoupons = await Coupon.find({
                    $and: [
                        { discount_percentage: { $gte: 1 } },
                        { discount_percentage: { $lte: 10 } }
                    ]
                }).lean();
            } else if (cartTotal >= 6000 && cartTotal <= 8000) {
                console.log('>=6000')
                availableCoupons = await Coupon.find({
                    $and: [
                        { discount_percentage: { $gte: 11 } },
                        { discount_percentage: { $lte: 20 } }
                    ]
                }).lean();
            } else if (cartTotal >= 8000 && cartTotal <= 10000) {
                console.log('>=8000')
                availableCoupons = await Coupon.find({
                    $and: [
                        { discount_percentage: { $gte: 25 } },
                        { discount_percentage: { $lte: 35 } }
                    ]
                }).lean();
            } else if (cartTotal >= 10000) {
                console.log('>=10000')
                availableCoupons = await Coupon.find({
                    discount_percentage: { $gte: 50 }
                }).lean();
            }
